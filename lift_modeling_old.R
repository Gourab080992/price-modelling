get.lift.model <- function(data, other.flag) {
  ...
  lift.model.data <- lift.model.data %>%
    select(LIFT_FRAC, TPR_DISC, TPR_DISC_SQ, MINOR_DISPLAY, MAJOR_DISPLAY, AD_DISPLAY)
  
  if (nrow(lift.model.data) < 12) {
    fit.data <- data.frame(c(NA), c(NA), c(NA), c(NA), c(NA), c(NA), c(NA), c(NA))
    colnames(fit.data) <- c("COEFF_A", "COEFF_B", "COEFF_C", "COEFF_MINOR_DISPLAY", "COEFF_MAJOR_DISPLAY",
                            "COEFF_AD_DISPLAY", "RSQUARED", "ADJRSQUARED")
    return(fit.data)
  }
  
  model <- lm(formula = LIFT_FRAC ~ TPR_DISC_SQ + TPR_DISC + MINOR_DISPLAY + MAJOR_DISPLAY + AD_DISPLAY - 1,
  ...
  COEFF_C <- 0
  COEFF_MINOR_DISPLAY <- c(model.param["MINOR_DISPLAY"])
  COEFF_MAJOR_DISPLAY <- c(model.param["MAJOR_DISPLAY"])
  COEFF_AD_DISPLAY <- c(model.param["AD_DISPLAY"])
  RSQUARED <- rsquared
  ADJRSQUARED <- adjrsquared
  
  model.param.df <- data.frame(COEFF_A, COEFF_B, COEFF_C, COEFF_MINOR_DISPLAY, COEFF_MAJOR_DISPLAY, COEFF_AD_DISPLAY,
...
run.lift <- function(raw.data, lift.dataclass, other.flag, lift.flag.condns) {
  ...
  raw.data <- raw.data %>%
    mutate(
      ...
      PRIMARY_TACTIC = if_else(PROMO_FLAG == 1 & 
                                     TPR_FLAG == 0 & 
                                     LIFT < 1.20, "MINOR_DISPLAY", NA_character_),
      
      PRIMARY_TACTIC = if_else(PROMO_FLAG == 1 & 
                                     TPR_FLAG == 0 & 
                                     1.20 <= LIFT & LIFT < 1.50, "MAJOR_DISPLAY", PRIMARY_TACTIC),
      ...
      PRIMARY_TACTIC = if_else(PROMO_FLAG == 1 & 
                                     TPR_FLAG == 1 & 
                                     PROMOELAS < 3.0, "MINOR_DISPLAY", PRIMARY_TACTIC),
      
      PRIMARY_TACTIC = if_else(PROMO_FLAG == 1 & 
                                     TPR_FLAG == 1 & 
                                     3.0 <= PROMOELAS & PROMOELAS < 6.0, "MAJOR_DISPLAY", PRIMARY_TACTIC),
      
      PRIMARY_TACTIC = if_else(PROMO_FLAG == 1 & 
                                     TPR_FLAG == 1 & 
                                     6.0 <= PROMOELAS, "MEGA_PROMO", PRIMARY_TACTIC),
      
      MINOR_DISPLAY = if_else(PRIMARY_TACTIC == "MINOR_DISPLAY", 1, 0),
      MAJOR_DISPLAY = if_else(PRIMARY_TACTIC == "MAJOR_DISPLAY", 1, 0),
  ...
  lift.model.data <- raw.data %>%
    ...
    summarise(
      ...
      MINOR_DISPLAY = first(MINOR_DISPLAY),
      MAJOR_DISPLAY = first(MAJOR_DISPLAY),
